#include "sp_mesh_loader.hpp"

const uint8_t VERSION_NOW = 1;
const int ENDIANNESS_TEST = 0x01000000;
const char* ENDIANNESS_TEST_PTR = (const char*)&ENDIANNESS_TEST;
...
scene::IAnimatedMesh* SPMeshLoader::createMesh(io::IReadFile* f)
{
    if (!IS_LITTLE_ENDIAN)
    {
        printf("Not little endian machine.\n");
        return NULL;
    }
    if (f == NULL)
    {
    ...
    {
        printf("Version mismatch, file %d SP %d\n", version,
            VERSION_NOW);
        m_mesh->drop();
        return NULL;
    }
    ...
    if (header == "SPMS")
    {
        printf("Space partitioned mesh not supported.");
        m_mesh->drop();
        return NULL;
    }
    f->read(&byte, 1);
    ...
    while (size_num != 0)
    {
        ...
        if (!tex_name_1.empty())
        {
            ...
            video::ITexture* tex = m_scene_manager->getVideoDriver()
                ->getTexture(tex_name_1.c_str());
            if (tex != NULL)
            {
                textures[0] = tex;
            }
        ...
        if (!tex_name_2.empty())
        {
            ...
            textures[1] =  m_scene_manager->getVideoDriver()
                ->getTexture(tex_name_1.c_str());
        }
        video::SMaterial m;
    ...
    while (size_num != 0)
    {
        ...
        while (mat_size != 0)
        {
            ...
            {
                printf("32bit index not supported.\n");
                m_mesh->drop();
                return NULL;
            }
...
void SPMeshLoader::Armature::read(irr::io::IReadFile* spm)
{
    ...
    if (!non_parent_bone)
    {
        printf("Non-parent bone missing in armature, exiting\n");
        assert(false);
    }
